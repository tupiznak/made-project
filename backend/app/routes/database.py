...
async def filter_database_papers(paper_filter: dict, exclude_paper: dict = None, chunk_size: int = 10):
    """
     ...
     ### В ответ на запрос выозвращается *chunk_size* статей, параметры которых включют параметры из *paper_filter* и исключают параметры из *exclude_paper*
...
async def sub_str_in_abstract_database_papers(sub_string: str, chunk_size: int = 10):
    """
     ## Запрос позвляет получить несколько статей из базы данных по подстроке в описании статьи.
        Для получения статей, в описании которых присутствует заданная подстрока, необходимо передать один обязательный параметр:
